...
def home():
    if request.method == 'POST':
        ...
        if 'submit-lang' in request.form:
             return redirect(url_for('views.homeFr'))
        ...
        if 'submit-message' in request.form:
            ...
            if not checkEmail(email):
                ...
                flash("Please enter a valid phone number, e.g. +44 1234 123456 or 01234 123456", category='error')
            elif len(message) < 10:
    ...
    return render_template("/en.html", user=current_user, reviews=getReviews())

@views.route('/fr', methods=['GET', 'POST'])
def homeFr():
    if request.method == 'POST':
    ...
    return render_template("/fr.html", user=current_user, reviews=getReviews())
